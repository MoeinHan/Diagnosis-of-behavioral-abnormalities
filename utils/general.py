...
os.environ['NUMEXPR_MAX_THREADS'] = str(min(os.cpu_count(), 0))  # NumExpr max threads
...
def feature_extractor():
    ...
    model = keras.applications.xception.Xception(input_shape=(500,400,3),
                                                        include_top=False,
                                                        weights='imagenet',
                                                        pooling='avg')
    preprocessing = keras.applications.xception.preprocess_input
    input_ = keras.Input((500,400,3))
    preprocessed = preprocessing(input_)

    output = model(preprocessed)
    return keras.Model(input_, output, name="feature_extractor")
